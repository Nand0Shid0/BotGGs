...
direccion_equipo = "192.168.0.17"


credenciales = ["cisco","cisco123!"]

ip_actuador = "http://192.168.1.103/"
...
class Menu(Command):
    ...
    def execute(self, message, attachment_actions, activity):
        ...
        **Change device name**
        ...
        Change name + new device name
            
        Example: Change name R2

        **Create loopback on the device.**

        To use this option you have to type:
        Create loopback + loopback number + ip address loopback + submasck loopback.

        Example: Create loopback 150 192.168.100.15 255.255.255.0

        **Delete loopback on the device.**

        To use this option you have to type:
        Delete loopback + loopback number

        Example: Delete loopback 150

        **Show interface on the device.**

        To use this option you have to type: Show interface

        **Actuador**
        As an extra option, the use of an actuator was
        added to the bot, to use it you just have to type:

         Led On --> Will send a signal to an esp 32 to turn on a led.

         Led Off --> Will send a signal to an esp 32 to turn off a led.
...
class Esp32(Command):
    ...
    def execute(self, message, attachment_actions, activity):
        ...
        if tmp == 'on'.lower():
            print("entre al on")
            payload_led = {'led':'on'}
            led_r = requests.get(url =ip_actuador, params = payload_led)
            return "LED ON"
        elif tmp == 'off'.lower():
            print("entre al off")
            payload_led = {'led':'off'}
